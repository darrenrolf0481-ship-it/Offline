      ...
      <div style={{ display: 'flex' }}>
          ...
          <div className={styles.detail_top}>
            <div style={{ display: 'flex' }}>
              ...
              >
                ...
                <div
                  className={styles.single_line_block}
                  style={
                    selectedRun && selectedRun.status === 'RUNNING'
                      ? { marginLeft: '7px', maxWidth: '100px' }
                      : { marginLeft: '-8px', maxWidth: '100px' }
                  }
                >
                  {selectedRun?.name || ''}
                </div>
              ...
              <div style={{ marginLeft: '7px' }}>
                <button
                  onClick={() => setLeftPanel('activity_feed')}
              ...
              {agentDetails && agentDetails.agent_type === 'Maintain Task Queue' && (
                <div style={{ marginLeft: '7px' }}>
                  <button
                    onClick={() => setLeftPanel('agent_type')}
                    className={styles.tab_button}
                    style={
            ...
            <div style={{ display: 'flex' }}>
              <div>
                <button className={styles.run_button} onClick={() => setRunModal(true)}>
                  <Image width={14} height={14} src="/images/run_icon.svg" alt="run-icon" />
                  &nbsp;New Run
                </button>
              </div>
              {selectedRun &&
                ...
                (selectedRun.status !== 'COMPLETED' ||
                  (selectedRun.status === 'COMPLETED' && agentExecutions.length > 1)) && (
                  <button
              ...
              {dropdown && (
                <div onMouseEnter={() => setDropdown(true)} onMouseLeave={() => setDropdown(false)}>
                  <ul className="dropdown_container">
                    ...
                    {selectedRun &&
                      (selectedRun.status === 'CREATED' || selectedRun.status === 'PAUSED') && (
                        <li
                          className="dropdown_item"
                          onClick={() => {
                    ...
                    {agentExecutions && agentExecutions.length > 1 && (
                      <li
                        className="dropdown_item"
                        onClick={() => {
          ...
          <div className={styles.detail_body}>
            {leftPanel === 'activity_feed' && (
              <div className={styles.detail_content}>
                <ActivityFeed selectedRunId={selectedRun?.id || 0} />
              </div>
            )}
            {leftPanel === 'agent_type' && (
              <div className={styles.detail_content}>
                <TaskQueue selectedRunId={selectedRun?.id || 0} />
              </div>
            )}
          </div>
        </div>
        <div style={{ width: '40%' }}>
            ...
            <div style={{ display: 'flex', overflowX: 'scroll' }}>
              ...
              <div>
                <button
                  ...
                  style={
                    rightPanel === 'details'
                      ? { background: '#454254', paddingRight: '15px' }
              ...
              <div>
                <button
                  onClick={() => setRightPanel('resource_manager')}
                  className={styles.tab_button}
                  style={
          ...
          <div className={styles.detail_body} style={{ paddingRight: '0' }}>
            {rightPanel === 'action_console' &&
              agentDetails &&
              agentDetails?.permission_type !== 'God Mode' && (
                <div className={styles.detail_content}>
                  <ActionConsole />
                </div>
              )}
            ...
            {rightPanel === 'resource_manager' && (
              <div className={styles.detail_content}>
                <ResourceManager agentId={agentId} />
              </div>
            )}
          </div>
        </div>
        ...
        {runModal && (
          <div className="modal" onClick={closeRunModal}>
