      ...
      <div className={styles.container}>
        ...
        <div className={styles.wrapper} style={{ marginBottom: '10px', marginTop: '4px' }}>
          <button
            ...
            onClick={() =>
              sendAgentData({ id: -1, name: 'new agent', contentType: 'Create_Agent' })
            }
          >
            + Create Agent
          </button>
        ...
        {agents && agents.length > 0 ? (
          <div className={styles.wrapper} style={{ overflowY: 'scroll', height: '80vh' }}>
            {agents.map((agent, index) => (
              <div key={index}>
                <div className={styles.agent_box} onClick={() => sendAgentData(agent)}>
                  {agent.status && (
                    <div className={styles.agent_active}>
                      <Image
                        width={14}
                        height={14}
                  ...
                  <div className={styles.text_block}>
                    <span className={styles.agent_text}>{agent.name}</span>
                  </div>
                </div>
              </div>
            ))}
