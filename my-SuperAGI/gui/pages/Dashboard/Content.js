  ...
  useEffect(() => {
    if (tabContainerRef.current) {
      ...
      if (tabElement) {
        ...
        if (
          tabOffsetLeft < containerScrollLeft ||
          tabOffsetLeft >= containerScrollLeft + containerWidth
        ) {
    ...
    <>
      <div style={{ display: 'flex', height: '100%' }}>
        <div
          ...
          style={selectedView === '' ? { width: '0vw' } : { width: '13vw' }}
        >
          {selectedView === 'agents' && (
            <div>
              <Agents sendAgentData={addTab} agents={agents} />
            </div>
          )}
          {selectedView === 'tools' && (
            <div>
              <Tools sendToolData={addTab} tools={tools} />
            </div>
          )}
        </div>
        {tabs.length <= 0 ? (
          <div
            className={styles.main_workspace}
            style={
              selectedView === '' ? { width: '93.5vw', paddingLeft: '10px' } : { width: '80.5vw' }
            }
          >
            <div className={styles.empty_state}>
              <div>
                ...
                >
                  <button
                    onClick={() =>
                      addTab({ id: -1, name: 'new agent', contentType: 'Create_Agent' })
                    }
          ...
          >
              ...
              <div className={styles.tabs} ref={tabContainerRef}>
                {tabs.map((tab) => (
                  <div
                    data-tab-id={tab.id}
                    key={tab.id}
                    className={`${styles.tab_box} ${selectedTab === tab.id ? styles.tab_box_selected : ''}`}
                  ...
                  >
                    <div style={{ display: 'flex', order: '0' }}>
                      {(tab.contentType === 'Agents' || tab.contentType === 'Create_Agent') && (
                        <div className={styles.tab_active}>
                          <Image
                            width={13}
                            height={13}
                            src="/images/agents_light.svg"
                            alt="agent-icon"
                          />
                        </div>
                      )}
                      {(tab.contentType === 'Tools' || tab.contentType === 'Create_Tool') && (
                        <div className={styles.tab_active}>
                          <Image
                            width={13}
                            height={13}
                            src="/images/tools_light.svg"
                            alt="tools-icon"
                          />
                        </div>
                      )}
                      ...
                      <div style={{ marginLeft: '8px' }}>
                        <span className={styles.tab_text}>{tab.name}</span>
                      </div>
                    </div>
                    <div
            ...
            <div
              className={styles.tab_detail}
              style={tabs.length > 0 ? { backgroundColor: '#2F2C40' } : {}}
            >
