      ...
      success: function (response) {
        ...
        function fillBanners($container, banners, className) {
          $.each(banners, function (index, banner) {
            ...
            $anchor.append($img);
            let $banner = $(
              '<div class="' + className + '-slide" data-banner="' + (index + 1) + '"></div>'
            );
...
class Dashboard {
  ...
  displayConfig(config) {
    try {
      ...
      html +=
        '<div class="config-label">Version:</div><div class="config-value">' +
        config.serena_version +
        '</div>';
      ...
      if (config.active_project.name && config.active_project.path) {
        ...
        html +=
          '<div class="config-value"><span title="Project configuration in ' +
          configPath +
          '">' +
      ...
      if (this.jetbrainsMode) {
        ...
        if (config.languages && config.languages.length > 0) {
          ...
          config.languages.forEach(function (language, index) {
            ...
            if (isRemovable) {
              html +=
                '<span class="language-remove" data-language="' + language + '">&times;</span>';
            }
            html += '</div>';
          });
      ...
      html +=
        '<div class="config-value"><span title="' +
        config.context.path +
        '">' +
        config.context.name +
        '</span></div>';
      ...
      html += '</h3>';
      html +=
        '<div class="collapsible-content tools-grid" id="tools-content" style="' +
        (wasToolsExpanded ? '' : 'display:none;') +
        ' margin-top: 10px;">';
      ...
      if (config.active_project && config.active_project.name) {
        ...
        html +=
          '<span class="toggle-icon' + (wasMemoriesExpanded ? ' expanded' : '') + '">▼</span>';
        html += '</h3>';
        html +=
          '<div class="collapsible-content memories-container" id="memories-content" style="' +
          (wasMemoriesExpanded ? '' : 'display:none;') +
          ' margin-top: 10px;">';
  ...
  displayAvailableTools(tools) {
    ...
    tools.forEach(function (tool) {
      html +=
        '<div class="info-item" title="' + tool.name + '"><code>' + tool.name + '</code></div>';
    });
  ...
  loadQueuedExecutions(onComplete) {
    ...
    $.ajax({
      ...
      error: function (xhr, status, error) {
        ...
        self.$activeExecutionQueueDisplay.html(
  ...
  loadLastExecution(onComplete) {
    ...
    $.ajax({
      ...
      error: function (xhr, status, error) {
        ...
        self.$lastExecutionDisplay.html(
  ...
  displayActiveExecutionsQueue(executions) {
    ...
    executions.forEach(function (execution) {
      ...
      html +=
        '<button class="execution-cancel-btn" data-task-id="' +
        execution.task_id +
        '" data-is-running="' +
        isRunning +
        '">✕</button>';
          ...
          if (response.was_cancelled) {
            ...
            console.log(
              'Task ' +
                ...
                ' could not be cancelled (may have already completed). ' +
                response.message
            );
          }
  ...
  saveLogs() {
    ...
    const checkmarkSvg =
      '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#888" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg><span class="log-action-btn-text">save logs</span>';
    this.$saveLogsBtn.html(checkmarkSvg);
    setTimeout(() => {
      this.$saveLogsBtn.html(originalHtml);
    }, 1500);
  }
  ...
  copyLogs() {
    ...
    navigator.clipboard
      ...
      .then(() => {
        ...
        const checkmarkSvg =
          '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#888" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg><span class="log-action-btn-text">copy logs</span>';
  ...
  clearLogs() {
    ...
    $.ajax({
      ...
      success: function () {
        ...
        const checkmarkSvg =
          '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#888" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg><span class="log-action-btn-text">clear logs</span>';
        self.$clearLogsBtn.html(checkmarkSvg);
        setTimeout(() => {
          self.$clearLogsBtn.html(originalHtml);
        }, 1500);
      },
      error: function (xhr, status, error) {
    ...
    $.ajax({
      ...
      error: function (xhr, status, error) {
        ...
        self.$errorContainer.html(
          '<div class="error-message">Error loading logs: ' +
            (xhr.responseJSON ? xhr.responseJSON.detail : error) +
            '</div>'
        );
      },
    });
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (languages.length === 0) {
          ...
          self.$modalLanguageSelect
            .prop('disabled', true)
            .attr('placeholder', 'No languages available');
  ...
  renderLanguageOptions(filter) {
    ...
    this.$modalLanguageOptions.empty();
    if (matches.length === 0) {
      this.$modalLanguageOptions.append($('<li class="no-match"></li>').text('No matches'));
    } else {
      matches.forEach(function (lang, idx) {
        ...
        const $li = $('<li role="option"></li>').attr('data-value', lang).html(label);
        if (idx === 0) {
  ...
  bindLanguageCombobox() {
    ...
    this.$modalLanguageSelect
      ...
      .on('focus.combobox click.combobox', function () {
    ...
    $(document)
      .off('mousedown.langCombobox')
      .on('mousedown.langCombobox', function (e) {
  ...
  commitMemoryRename() {
    ...
    if (!/^[a-zA-Z0-9_]+(?:\/[a-zA-Z0-9_]+)*$/.test(newName)) {
      alert(
        'Memory name can only contain letters, numbers, underscores, and "/" for subdirectories (e.g., "topic/memory_name")'
      );
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (
          ...
          response.news &&
          Object.keys(response.news).length > 0
        ) {
  ...
  displayNews(newsIds, newsData) {
    ...
    newsIds.forEach(function (newsId) {
      ...
      let $button = $('<button class="news-mark-read-btn">')
  ...
  saveSerenaConfigFromModal() {
    ...
    $.ajax({
      ...
      error: function (xhr, status, error) {
        ...
        alert(
          'Error saving serena config: ' + (xhr.responseJSON ? xhr.responseJSON.message : error)
        );
      },
      complete: function () {
        // Re-enable button
