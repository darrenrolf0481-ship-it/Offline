...
async function startServer() {
    ...
    createProxyMiddleware({
      ...
      on: {
        error: (err: any, req: any, res: any) => {
          ...
          res.end(
            JSON.stringify({ error: 'Ollama is not running or accessible', details: err.message })
          );
        },
      },
    })
  ...
  app.post('/api/setup-mcp-github', async (req, res) => {
    try {
      ...
      if (tools.length > 0) {
        ...
        return res.json({
          status: 'success',
          message: `Connected to GitHub MCP. Loaded ${tools.length} tools.`,
        });
      } else {
    ...
    try {
      ...
      const sanitizedPayload =
        typeof payload === 'string'
          ? payload
              .replace(new RegExp(sourceAgentId, 'g'), '[SND_SEQ]')
  ...
  app.post('/query_project', async (req, res) => {
    ...
    try {
      ...
      const params =
        typeof tool_params_json === 'string' ? JSON.parse(tool_params_json) : tool_params_json;
