...
export const filesystemTools = [
  {
    name: "filesystem",
    description: "Unified filesystem tool for all file and directory operations. Supports read, write, list, search, info, and create_directory actions.",
    inputSchema: z.object({
      action: z.enum(["read", "write", "list", "search", "info", "create_directory"]).describe("Filesystem action to perform"),
      ...
      recursive: z.boolean().describe("List subdirectories recursively (for list action)").default(false).optional(),
      pattern: z.string().describe("Glob pattern to search for (for search action)").optional(),
      ignore: z.array(z.string()).describe("Patterns to ignore (for search action)").default(["node_modules/**", ".git/**"]).optional(),
    }),
        ...
        switch (args.action) {
          ...
          case "write": {
            if (!args.path || args.content === undefined) {
              return formatToolResponse({ success: false, error: "path and content required for write action" });
            }
            const validPath = validatePath(args.path);
            ...
            if (args.recursive) {
              ...
              for (const subdir of subdirs) {
                ...
                if (subResult.content?.[0]?.text) {
                  ...
                  if (parsed.items) {
                    items = [...items, ...parsed.items.map((item: any) => ({
                      ...item,
          ...
          case "search": {
            if (!args.pattern) {
              return formatToolResponse({ success: false, error: "pattern required for search action" });
            }
          ...
          case "create_directory": {
            if (!args.path) {
              return formatToolResponse({ success: false, error: "path required for create_directory action" });
            }
            const validPath = validatePath(args.path);
