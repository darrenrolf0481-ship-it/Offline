...
export const gitTools = [
  {
    name: "git",
    description: "Unified git tool for all version control operations. Supports status, log, diff, branch, commit, push, pull, clone, and stash actions.",
    inputSchema: z.object({
      action: z.enum(["status", "log", "diff", "branch", "commit", "push", "pull", "clone", "stash"]).describe("Git action to perform"),
      ...
      branchAction: z.enum(["list", "create", "switch", "delete"]).describe("Branch operation").optional(),
      ...
      try {
        switch (args.action) {
          ...
          case "clone": {
            ...
            const targetPath = args.directory ? path.join(workspacePath, args.directory) : workspacePath;
