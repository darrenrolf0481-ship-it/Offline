...
export const ragTools = [
  {
    ...
    description:
      'Index local documents for RAG retrieval. Requires RAG_DOCS_PATH environment variable to be set. This will scan the specified directory, chunk documents, and build a TF-IDF index for semantic search.',
    inputSchema: z.object({
      forceReindex: z
        .boolean()
        .describe('Force reindexing of all documents even if unchanged')
        .default(false)
        .optional(),
      filePatterns: z
        .array(z.string())
        .describe("Optional glob patterns to filter files (e.g., ['**/*.md', '**/*.ts'])")
        .optional(),
    }),
    handler: async (args: any) => {
      if (!isRAGEnabled()) {
        return formatToolResponse({
          success: false,
          error:
            'RAG is not enabled. Set RAG_DOCS_PATH environment variable to the directory containing documents to index.',
        });
      }
  ...
  },
  {
    name: 'rag_search',
    description:
      'Search indexed documents using semantic similarity. Returns the most relevant document chunks based on the query. Requires documents to be indexed first using rag_index_documents.',
    inputSchema: z.object({
      ...
      fileFilter: z
        .string()
        .describe('Optional regex pattern to filter results by file path')
        .optional(),
    }),
    handler: async (args: any) => {
      if (!isRAGEnabled()) {
  ...
  },
  {
    ...
    description:
      'Get expanded context around a specific search result. Useful for getting more surrounding content from a document after finding a relevant chunk.',
    inputSchema: z.object({
      ...
      expandLines: z
        .number()
        .describe('Number of lines to expand before and after (default: 20)')
        .default(20)
        .optional(),
    }),
    handler: async (args: any) => {
  ...
  },
  {
    ...
    description:
      'List all indexed documents and their metadata. Shows what files are currently in the RAG index.',
    inputSchema: z.object({
      showChunks: z
        .boolean()
        .describe('Include chunk details for each file (default: false)')
        .default(false)
        .optional(),
    }),
    handler: async (args: any) => {
  ...
  },
  {
    ...
    description:
      'Clear the RAG index. This removes all indexed documents and requires re-indexing.',
    inputSchema: z.object({
  ...
  },
  {
    name: 'rag_status',
    description:
      "Check the status of the RAG system, including whether it's enabled and index statistics.",
    inputSchema: z.object({}),
    handler: async () => {
