...
export const terraformTools = [
  {
    name: 'terraform',
    description:
      'Unified Terraform tool for CLI operations, registry documentation lookup, and state management. Supports init, plan, apply, destroy, validate, fmt, state operations, and documentation queries.',
    inputSchema: z.object({
      ...
      directory: z
        .string()
        .describe('Working directory for Terraform (relative to workspace)')
        .default('.')
        .optional(),
      autoApprove: z
        .boolean()
        .describe('Auto-approve apply/destroy operations')
        .default(false)
        .optional(),
      ...
      resourceType: z
        .string()
        .describe("Resource type (e.g., 'aws_instance', 'google_compute_instance')")
        .optional(),
      functionName: z
        .string()
        .describe("Function name (e.g., 'lookup', 'merge', 'jsonencode')")
        .optional(),

      // Format options
      check: z
        .boolean()
        .describe('Check if files are formatted (fmt only)')
        .default(false)
        .optional(),
      recursive: z
        .boolean()
        .describe('Process subdirectories recursively (fmt only)')
        .default(false)
        .optional(),
    }),
      ...
      try {
        switch (args.action) {
          ...
          case 'workspace_new': {
            ...
            const { stdout } = await execAsync(`terraform workspace new "${args.workspace}"`, {
              cwd,
            });
            return formatToolResponse({
              success: true,
          ...
          case 'workspace_select': {
            ...
            const { stdout } = await execAsync(`terraform workspace select "${args.workspace}"`, {
              cwd,
            });
            return formatToolResponse({
              success: true,
          ...
          case 'docs_search': {
            ...
            return formatToolResponse({
              ...
              message:
                'Visit the Terraform Registry to search for providers, modules, and documentation',
              quickLinks: {
