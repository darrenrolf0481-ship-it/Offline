...
const gcloud_auth_login = {
  ...
  description: "Authenticate with Google Cloud using Application Default Credentials (ADC). This will open a browser for OAuth flow.",
  inputSchema: z.object({
    force: z.boolean().optional().default(false).describe("Force re-authentication even if already logged in"),
  }),
...
const gcloud_auth_print_identity_token = {
  ...
  description: "Generate and print an identity token (JWT) for the active account. Used for authenticating to Cloud Run and other services.",
  inputSchema: z.object({
...
const gcloud_config_set = {
  ...
  inputSchema: z.object({
    property: z.string().describe("Property to set (e.g., 'project', 'compute/region', 'compute/zone')"),
...
const gcloud_config_get = {
  ...
  inputSchema: z.object({
    property: z.string().describe("Property to get (e.g., 'project', 'compute/region', 'compute/zone')"),
  }),
  handler: async (args: any) => {
    try {
...
const gcloud_logging_read = {
  ...
  inputSchema: z.object({
    filter: z.string().optional().describe("Log filter query (e.g., 'resource.type=cloud_run_revision')"),
    limit: z.number().optional().default(50).describe("Maximum number of log entries to return"),
    freshness: z.string().optional().describe("Return logs newer than this duration (e.g., '1h', '30m', '7d')"),
  ...
  handler: async (args: any) => {
    try {
      ...
      const result = await executeGcloudCommand(command);
      console.error(`[gcloud_logging_read] Result type: ${typeof result}, isArray: ${Array.isArray(result)}, length: ${Array.isArray(result) ? result.length : 'N/A'}`);
...
const gcloud_logging_write = {
  ...
  inputSchema: z.object({
    ...
    severity: z.enum(["DEFAULT", "DEBUG", "INFO", "NOTICE", "WARNING", "ERROR", "CRITICAL", "ALERT", "EMERGENCY"]).optional().default("INFO").describe("Log severity level"),
    resource_type: z.string().optional().default("global").describe("Resource type (e.g., 'cloud_run_revision', 'gce_instance', 'global')"),
  }),
  handler: async (args: any) => {
    try {
...
const gcloud_bq_query = {
  ...
  inputSchema: z.object({
    ...
    use_legacy_sql: z.boolean().optional().default(false).describe("Use legacy SQL instead of standard SQL"),
...
const gcloud_bq_ls = {
  ...
  inputSchema: z.object({
    dataset: z.string().optional().describe("Dataset name to list tables from (if not provided, lists datasets)"),
...
const gcloud_bq_show = {
  ...
  inputSchema: z.object({
    resource: z.string().describe("Resource to describe (format: 'project:dataset' or 'project:dataset.table')"),
  }),
  handler: async (args: any) => {
    try {
...
const gcloud_bq_mk = {
  ...
  inputSchema: z.object({
    ...
    schema: z.string().optional().describe("Table schema (for table creation, format: 'field1:type1,field2:type2')"),
...
const gcloud_dataflow_jobs_list = {
  ...
  inputSchema: z.object({
    ...
    status: z.enum(["active", "terminated", "all"]).optional().default("all").describe("Filter by job status"),
  }),
  handler: async (args: any) => {
    try {
...
const gcloud_dataflow_jobs_describe = {
  ...
  inputSchema: z.object({
    ...
    region: z.string().optional().default("us-central1").describe("Region where the job is running"),
  }),
  handler: async (args: any) => {
    try {
...
const gcloud_services_list = {
  ...
  inputSchema: z.object({
    project: z.string().optional().describe("Project ID (uses current project if not specified)"),
    available: z.boolean().optional().default(false).describe("List available services instead of enabled ones"),
  }),
  handler: async (args: any) => {
    try {
...
const gcloud_run_services_list = {
  ...
  inputSchema: z.object({
    region: z.string().optional().describe("Filter by region (e.g., 'us-central1')"),
    platform: z.enum(["managed", "gke", "kubernetes"]).optional().default("managed").describe("Cloud Run platform"),
  }),
  handler: async (args: any) => {
    try {
...
const gcloud_storage_ls = {
  ...
  inputSchema: z.object({
    path: z.string().describe("GCS path to list (e.g., 'gs://bucket-name' or 'gs://bucket-name/path/')"),
...
const gcloud_execute = {
  name: "gcloud_execute",
  description: "Execute any gcloud command directly. This is a flexible wrapper that allows running any gcloud command with proper ADC authentication. Use this for commands not covered by specific tools.",
  inputSchema: z.object({
    command: z.string().describe("The gcloud command to execute (without 'gcloud' prefix, e.g., 'compute instances list')"),
    format: z.enum(["json", "yaml", "text", "default"]).optional().default("json").describe("Output format"),
