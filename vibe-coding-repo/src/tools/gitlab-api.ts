...
function checkGitLabAuth(): string {
  ...
  if (!token) {
    throw new Error(
      'GITLAB_API_KEY environment variable is not set. Please set it to use GitLab tools.'
    );
  }
  return token;
}
...
export const gitlab_get_group = {
  ...
  description:
    'Get detailed information about a GitLab group including description, visibility, and statistics',
  inputSchema: z.object({
    group_id: z
      .string()
      .describe("Group ID or URL-encoded group path (e.g., 'my-group' or 'parent/child')"),
...
export const gitlab_list_group_subgroups = {
  ...
  inputSchema: z.object({
    ...
    all_available: z
      ...
      .optional()
      .default(false)
      .describe('Show all groups you have access to'),
  ...
  handler: async (args: any) => {
    try {
      ...
      const data = await gitlabGet(
        `${GITLAB_API_BASE}/groups/${encodedGroupId}/subgroups?${params}`,
        token
      );
      return formatToolResponse(data);
    } catch (error) {
...
export const gitlab_list_group_projects = {
  ...
  handler: async (args: any) => {
    try {
      ...
      const data = await gitlabGet(
        `${GITLAB_API_BASE}/groups/${encodedGroupId}/projects?${params}`,
        token
      );
      return formatToolResponse(data);
    } catch (error) {
...
export const gitlab_list_descendant_groups = {
  ...
  description:
    'List all descendant groups (all nested subgroups at any level) under a GitLab group',
  inputSchema: z.object({
    group_id: z.string().describe('Group ID or URL-encoded group path'),
  ...
  handler: async (args: any) => {
    try {
      ...
      const data = await gitlabGet(
        `${GITLAB_API_BASE}/groups/${encodedGroupId}/descendant_groups?${params}`,
        token
      );
      return formatToolResponse(data);
    } catch (error) {
...
export const gitlab_get_project = {
  ...
  inputSchema: z.object({
    project_id: z
      .string()
      .describe("Project ID or URL-encoded project path (e.g., 'namespace/project')"),
  }),
  handler: async (args: any) => {
    try {
      const token = checkGitLabAuth();
...
export const gitlab_list_projects = {
  ...
  inputSchema: z.object({
    ...
    membership: z
      ...
      .optional()
      .default(false)
      .describe('Only show projects user is member of'),
    archived: z.boolean().optional().describe('Filter by archived status'),
    visibility: z
      .enum(['public', 'internal', 'private'])
      .optional()
...
export const gitlab_list_issues = {
  ...
  handler: async (args: any) => {
    try {
      ...
      const data = await gitlabGet(
        `${GITLAB_API_BASE}/projects/${encodedProjectId}/issues?${params}`,
        token
      );
      return formatToolResponse(data);
    } catch (error) {
...
export const gitlab_get_issue = {
  ...
  handler: async (args: any) => {
    try {
      ...
      const data = await gitlabGet(
        `${GITLAB_API_BASE}/projects/${encodedProjectId}/issues/${args.issue_iid}`,
        token
      );
      return formatToolResponse(data);
    } catch (error) {
...
export const gitlab_update_issue = {
  ...
  inputSchema: z.object({
    ...
    labels: z
      .string()
      .optional()
      .describe('Comma-separated list of label names (replaces existing)'),
    milestone_id: z.number().optional().describe('Milestone ID to assign'),
    assignee_ids: z.array(z.number()).optional().describe('Array of user IDs to assign'),
...
export const gitlab_list_issue_notes = {
  ...
  handler: async (args: any) => {
    try {
      ...
      const data = await gitlabGet(
        `${GITLAB_API_BASE}/projects/${encodedProjectId}/issues/${args.issue_iid}/notes?${params}`,
        token
      );
      return formatToolResponse(data);
    } catch (error) {
...
export const gitlab_list_merge_requests = {
  ...
  handler: async (args: any) => {
    try {
      ...
      const data = await gitlabGet(
        `${GITLAB_API_BASE}/projects/${encodedProjectId}/merge_requests?${params}`,
        token
      );
      return formatToolResponse(data);
    } catch (error) {
...
export const gitlab_get_merge_request = {
  ...
  inputSchema: z.object({
    ...
    include_diverged_commits_count: z
      ...
      .optional()
      .default(false)
      .describe('Include diverged commits count'),
    include_rebase_in_progress: z
      .boolean()
      .optional()
      .default(false)
      .describe('Include rebase in progress status'),
  }),
  handler: async (args: any) => {
    try {
      const token = checkGitLabAuth();
      const encodedProjectId = encodeURIComponent(args.project_id);
      const params = new URLSearchParams();
      if (args.include_diverged_commits_count)
        params.append('include_diverged_commits_count', 'true');
      if (args.include_rebase_in_progress) params.append('include_rebase_in_progress', 'true');

      const data = await gitlabGet(
        `${GITLAB_API_BASE}/projects/${encodedProjectId}/merge_requests/${args.merge_request_iid}?${params}`,
        token
      );
      return formatToolResponse(data);
    } catch (error) {
...
export const gitlab_merge_merge_request = {
  ...
  handler: async (args: any) => {
    try {
      ...
      if (args.should_remove_source_branch !== undefined)
        body.should_remove_source_branch = args.should_remove_source_branch;
      if (args.merge_when_pipeline_succeeds)
...
export const gitlab_list_mr_changes = {
  ...
  handler: async (args: any) => {
    try {
      ...
      const data = await gitlabGet(
        `${GITLAB_API_BASE}/projects/${encodedProjectId}/merge_requests/${args.merge_request_iid}/changes`,
        token
      );
      return formatToolResponse(data);
    } catch (error) {
...
export const gitlab_list_mr_notes = {
  ...
  handler: async (args: any) => {
    try {
      ...
      const data = await gitlabGet(
        `${GITLAB_API_BASE}/projects/${encodedProjectId}/merge_requests/${args.merge_request_iid}/notes?${params}`,
        token
      );
      return formatToolResponse(data);
    } catch (error) {
...
export const gitlab_search_issues_global = {
  ...
  description:
    'Search issues across all accessible projects. Use this to find issues assigned to a user across all projects.',
  inputSchema: z.object({
    assignee_username: z.string().optional().describe('Filter by assignee username'),
    author_username: z.string().optional().describe('Filter by author username'),
    state: z
...
export const gitlab_search_merge_requests_global = {
  ...
  description:
    'Search merge requests across all accessible projects. Use this to find MRs assigned to or authored by a user.',
  inputSchema: z.object({
    assignee_username: z.string().optional().describe('Filter by assignee username'),
    author_username: z.string().optional().describe('Filter by author username'),
    reviewer_username: z.string().optional().describe('Filter by reviewer username'),
...
export const gitlab_search_group_issues = {
  ...
  handler: async (args: any) => {
    try {
      ...
      const data = await gitlabGet(
        `${GITLAB_API_BASE}/groups/${encodedGroupId}/issues?${params}`,
        token
      );
      return formatToolResponse(data);
    } catch (error) {
...
export const gitlab_search_group_merge_requests = {
  ...
  handler: async (args: any) => {
    try {
      ...
      const data = await gitlabGet(
        `${GITLAB_API_BASE}/groups/${encodedGroupId}/merge_requests?${params}`,
        token
      );
      return formatToolResponse(data);
    } catch (error) {
