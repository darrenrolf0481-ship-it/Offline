...
export const diagnosticsTools = [
  {
    ...
    inputSchema: z.object({
      filePath: z
        .string()
        .describe('Optional file path to filter problems for specific file')
        .optional(),
      severity: z
    ...
    handler: async (args: any) => {
      try {
        ...
        if (args.filePath) {
          filteredProblems = problems.filter((p: any) => p.file.includes(args.filePath));
        }

        // Filter by severity
        if (args.severity !== 'all') {
          filteredProblems = filteredProblems.filter((p: any) => p.severity === args.severity);
        }
        ...
        return formatToolResponse({
          ...
          summary: {
            ...
            warnings: filteredProblems.filter((p: any) => p.severity === 'warning').length,
  ...
  },
  {
    ...
    inputSchema: z.object({
      ...
      lines: z
        .number()
        .describe('Number of lines to read from end (tail -n)')
        .default(100)
        .optional(),
    ...
    handler: async (args: any) => {
      try {
        ...
        if (args.level) {
          lines = lines.filter(
            (line) =>
              line.includes(args.level) || line.toLowerCase().includes(args.level.toLowerCase())
          );
        }
  ...
  },
  {
    ...
    inputSchema: z.object({
      ...
      includeNodeModules: z
        .boolean()
        .describe('Include logs in node_modules')
        .default(false)
        .optional(),
    }),
    handler: async (args: any) => {
  ...
  },
  {
    ...
    inputSchema: z.object({
      ...
      extractStackTraces: z
        .boolean()
        .describe('Extract and parse stack traces')
        .default(true)
        .optional(),
    }),
    handler: async (args: any) => {
  ...
  },
  {
    ...
    inputSchema: z.object({
      ...
      lastReadPosition: z
        .number()
        .describe('Last byte position read (for incremental reads)')
        .default(0)
        .optional(),
      maxBytes: z
        .number()
        .describe('Maximum bytes to read from current position')
        .default(10240)
        .optional(),
    }),
    handler: async (args: any) => {
      try {
        ...
        const startPosition =
          args.lastReadPosition || Math.max(0, stats.size - (args.maxBytes || 10240));
  ...
  },
  {
    ...
    inputSchema: z.object({
      logPattern: z
        .string()
        .describe('Glob pattern to match log files')
        .default('**/*.log')
        .optional(),
      timeRange: z
        .object({
      ...
      groupBy: z
        ...
        .describe('How to group log entries')
        .default('level')
        .optional(),
    }),
    handler: async (args: any) => {
  ...
  },
  {
    ...
    inputSchema: z.object({
      ...
      logDir: z
        .string()
        .describe('Directory to search for logs (default: workspace root)')
        .default('.')
        .optional(),
      filePattern: z
        .string()
        .describe('File pattern for log files (default: *.log)')
        .default('*.log')
        .optional(),
      contextLines: z
        .number()
  ...
  },
  {
    ...
    inputSchema: z.object({
      logType: z
        ...
        .describe('Type of Xcode logs to retrieve')
        .default('all')
        .optional(),
          ...
          try {
            ...
            if (stdout) {
              const filteredLines = stdout
                .split('\n')
                .filter(
                  (line) => !args.filter || line.toLowerCase().includes(args.filter.toLowerCase())
                );
  ...
  },
  {
    ...
    inputSchema: z.object({
      logType: z
        ...
        .describe('Type of Android logs to retrieve')
        .default('logcat')
        .optional(),
      priority: z
        .enum(['V', 'D', 'I', 'W', 'E', 'F', 'all'])
        .describe('Logcat priority level (Verbose, Debug, Info, Warning, Error, Fatal)')
        .default('all')
        .optional(),
  ...
  },
  {
    ...
    inputSchema: z.object({
      logType: z
        ...
        .describe('Type of React Native logs to retrieve')
        .default('all')
        .optional(),
      lines: z.number().describe('Number of recent log lines to retrieve').default(100).optional(),
    }),
      ...
      try {
        ...
        if (args.logType === 'metro' || args.logType === 'all') {
          try {
            ...
            const { stdout } = await execAsync(
              "pgrep -fl 'react-native.*start' || pgrep -fl 'metro'"
            );
