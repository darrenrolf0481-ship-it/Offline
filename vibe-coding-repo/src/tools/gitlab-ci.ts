...
function checkGitLabAuth(): string {
  ...
  if (!token) {
    throw new Error(
      'GITLAB_API_KEY environment variable is not set. Please set it to use GitLab CI/CD tools.'
    );
  }
  return token;
}
...
export const gitlab_list_pipelines = {
  ...
  description:
    'List pipelines for a GitLab project with optional filters for status, ref, and username',
...
export const gitlab_get_pipeline = {
  ...
  description:
    'Get detailed information about a specific pipeline including status, duration, and commit details',
  inputSchema: z.object({
...
export const gitlab_list_pipeline_jobs = {
  ...
  inputSchema: z.object({
    ...
    scope: z
      ...
      .optional()
      .describe('Filter jobs by scope (can specify multiple)'),
  }),
  handler: async (args: any) => {
    try {
      const token = checkGitLabAuth();
...
export const gitlab_get_job = {
  name: 'gitlab_get_job',
  description:
    'Get detailed information about a specific job including status, duration, artifacts, and runner details',
  inputSchema: z.object({
    project_id: z.string().describe('Project ID or URL-encoded project path'),
