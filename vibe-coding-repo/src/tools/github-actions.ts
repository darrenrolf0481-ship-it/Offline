...
function checkGitHubAuth(): string {
  ...
  if (!token) {
    throw new Error(
      'GITHUB_API_KEY environment variable is not set. Please set it to use GitHub Actions tools.'
    );
  }
  return token;
}
...
export const github_list_workflow_runs = {
  ...
  description:
    'List workflow runs for a repository with optional filters for branch, status, event, and actor',
...
export const github_get_workflow_run = {
  ...
  description:
    'Get details of a specific workflow run including status, conclusion, and timing information',
  inputSchema: z.object({
    owner: z.string().describe('Repository owner'),
    repo: z.string().describe('Repository name'),
...
export const github_list_workflow_jobs = {
  ...
  inputSchema: z.object({
    ...
    filter: z
      ...
      .optional()
      .default('latest')
      .describe('Filter jobs to latest or all attempts'),
  }),
  handler: async (args: any) => {
    try {
      const token = checkGitHubAuth();
