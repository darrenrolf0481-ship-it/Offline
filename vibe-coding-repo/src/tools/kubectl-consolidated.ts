...
export const kubernetesTools = [
  {
    name: "kubectl",
    description: "Unified kubectl tool for Kubernetes operations. Supports get_pods, describe_pod, get_logs, get_deployments, get_services, get_events, and get_resource_status actions.",
    inputSchema: z.object({
      action: z.enum(["get_pods", "describe_pod", "get_logs", "get_deployments", "get_services", "get_events", "get_resource_status"]).describe("Kubectl action to perform"),
      ...
      allNamespaces: z.boolean().describe("Get resources from all namespaces").default(false).optional(),
        ...
        switch (args.action) {
          ...
          case "get_resource_status": {
            if (!args.resourceType || !args.name) {
              return formatToolResponse({ success: false, error: "resourceType and name parameters required" });
            }
            const { stdout } = await execAsync(
